...
export default function NotarizationPage() {
  ...
  const handleNotarize = async () => {
    ...
    try {
      ...
      setCertificateData({ fileHash, timestamp, owner });
      setResult("");
    } catch (error) {
  ...
  return certificateData ? (
    ...
    <div
      ...
      onDrop={handleDrop}
      className="p-6 max-w-xl mx-auto bg-white rounded-xl shadow-md space-y-4"
    >
      <h2 className="text-xl font-semibold text-gray-900">Notarize a File</h2>

      <input type="file" onChange={handleFileChange} className="block" />

      <button onClick={handleUpload} disabled={isLoading} className="btn btn-blue">
        Generate Hash
      </button>

      <button onClick={handleNotarize} disabled={isLoading} className="btn btn-green">
        Notarize File
      </button>

      <button onClick={handleVerify} disabled={isLoading} className="btn btn-purple">
        Verify Document
      </button>

      {fileHash && (
        <p className="text-sm break-all">Hash: {fileHash}</p>
      )}
